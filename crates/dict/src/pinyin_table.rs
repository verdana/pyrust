...
/// Pinyin syllable table: knows which strings are valid pinyin syllables.
...
impl PinyinTable {
    ...
    /// Check if `s` is a complete pinyin syllable.
    pub fn is_syllable(&self, s: &str) -> bool {
        self.trie.contains(s)
    }

    /// Starting from `start` in `input`, find all valid syllables.
    /// Returns (end_byte_offset, ()) pairs.
...
// ---------------------------------------------------------------------------
// Complete list of ~405 Mandarin pinyin syllables (without tone).
// ---------------------------------------------------------------------------
const ALL_SYLLABLES: &[&str] = &[
    "a", "ai", "an", "ang", "ao",
